export default function MapItem({mapId, mapName, mapArtist, mapper, imageUrl, mapCount=0, tabletype}) {
    ...
    return (
      <a href={`https://osu.ppy.sh/beatmapsets/${mapId}`} className="mx-auto max-w-screen-xl w-11/12 my-2 bg-">
        <div
          className={`relative flex items-center justify-between h-20 rounded-lg px-6 font-visby font-bold text-2xl text-opacity-80 shadow-osuslate-200/20 shadow-lg text-white border-r-4 ${borderColorClass} hover:-translate-y-0.5 hover:shadow-lg transition-all duration-300 hover:opacity-80 overflow-hidden`}>
            <div
