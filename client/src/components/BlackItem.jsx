export default function BlackItem({mapId, mapName, mapArtist, mapper, weeklyCount=0, monthlyCount=0, yearlyCount=0, alltimeCount=0}) {    
    return (
      <a href={`https://osu.ppy.sh/beatmapsets/${mapId}`} className="mx-auto max-w-screen-xl w-11/12 my-2">
        <div className={`relative flex items-center justify-between h-20 rounded-lg px-6 pr-3 font-visby font-bold text-2xl text-opacity-80 shadow-osuslate-200/20 shadow-lg text-white border-r-4 border-black hover:-translate-y-0.5 hover:shadow-lg transition-all duration-300 hover:opacity-80 overflow-hidden grayscale`}>
          <div
